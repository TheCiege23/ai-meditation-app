...
import { wellnessTopics, getWellnessTopicBySlug } from "@/data/seo/wellnessTopics";
import { buildTopicMetadata } from "@/lib/seo/metadataBuilder";
import { getTopicsBySlugs, topicHref } from "@/lib/seo/topicResolver";
...
export async function generateStaticParams() {
  return wellnessTopics.map((topic) => ({ slug: topic.slug }));
}
...
export async function generateMetadata(
  ...
  const topic = getWellnessTopicBySlug(params.slug);
  if (!topic) return {};
  return buildTopicMetadata(topic, "/wellness");
}

export default function WellnessTopicPage({ params }: Props) {
  const topic = getWellnessTopicBySlug(params.slug);
  if (!topic) notFound();

  const baseUrl =
    process.env.NEXT_PUBLIC_APP_URL?.replace(/\/$/, "") ?? "https://chimaura.com";
  const pageUrl = `${baseUrl}/wellness/${topic.slug}`;
  ...
  const hasBenefits = topic.benefits && topic.benefits.length > 0;
  const hasWhenToUse = topic.whenToUse && topic.whenToUse.length > 0;

  const primarySessionUrl = "/meditation";
    ...
    <main className="app-bottom-nav-pad min-h-screen bg-slate-100 text-slate-900 dark:bg-slate-950 dark:text-slate-100">
      <div className="mx-auto flex max-w-5xl flex-col px-4 py-6 sm:px-6 sm:py-8 lg:px-8">
        ...
        <header className="mb-6 flex items-center justify-between gap-4">
          ...
          >
            Meditation hub / Centro de meditación
          </Link>
        </header>
        ...
        <section className="mb-8 grid gap-6 md:grid-cols-[minmax(0,1.3fr)_minmax(0,0.9fr)]">
          <div className="rounded-3xl bg-white p-6 text-sm shadow-sm dark:bg-slate-900">
            ...
            <p className="mt-2 text-sm text-slate-700 dark:text-slate-300">
              Use this wellness page as a gentle starting point. Pick one short meditation or
              breathing pattern that matches how you feel, then let that be enough for today.
            </p>
            <Link
              href={primarySessionUrl}
          ...
          <div className="space-y-4 text-sm">
            <div className="rounded-3xl bg-white p-5 shadow-sm dark:bg-slate-900">
              <h2 className="text-base font-semibold text-slate-900 dark:text-slate-100">
                Related meditation & breathing
              </h2>
              <ul className="mt-2 space-y-1">
                {relatedTopics.map((related) => (
                  <li key={related.slug}>
                    <Link
                      href={topicHref(related)}
            ...
            <div className="rounded-3xl bg-white p-5 shadow-sm dark:bg-slate-900">
              ...
              <p className="mt-2 text-sm text-slate-700 dark:text-slate-300">
                Premium helps you deepen these wellness themes with longer sessions, sound
                mixing, and weekly reflection prompts.
              </p>
              <Link
                href="/pricing"
